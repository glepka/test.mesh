...
function DoEntireFormat(event, eventType, elemId, rawInput, pattern, mask) {
  ...
  if (
    ...
    !mask ||
    rawInput.length == 0 ||
    rawInput.length > pattern.length + 1
  ) {
    if (mask) {
      return {
        value: mask,
  ...
  while (formatted.length < pattern.length) {
    ...
    var nextCharMatchingRegEx = regExs.filter(
      (x) => x.patternVal == nextPatternChar
    );
    var nextNextCharMatchingRegEx = regExs.filter(
      (x) => x.patternVal == nextNextPatternChar
    );
    ...
    if (matchingRegEx.length == 1) {
      ...
      if (nextCharMatchingRegEx.length == 1) {
        if (nextCharMatchingRegEx[0].regEx.test(toBeFormatted[0])) {
          toBeFormatted =
            toBeFormatted.substring(0, 1) +
            toBeFormatted.substring(2, pattern.length - formatted.length);
          formatted += toBeFormatted[0];
          toBeFormatted = toBeFormatted.substring(
            2,
            pattern.length - formatted.length
          );
          startPosition++;
        }
        ...
        if (nextNextCharMatchingRegEx[0].regEx.test(toBeFormatted[0])) {
          ...
          toBeFormatted =
            toBeFormatted.substring(0, 1) +
            toBeFormatted.substring(3, pattern.length - formatted.length);
          formatted += toBeFormatted[0];
          startPosition++;
          toBeFormatted = toBeFormatted.substring(
            2,
